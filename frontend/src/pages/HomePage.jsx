    ...
    <div className="min-h-screen bg-gray-100 p-6 font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Link to="/sales" className="bg-indigo-600 hover:bg-indigo-700 text-white p-6 rounded-xl shadow-sm transition-colors text-center font-semibold flex flex-col items-center justify-center">
            <span className="text-2xl mb-2">🛒</span>
            New Sale
          </Link>
          <Link to="/inventory" className="bg-emerald-600 hover:bg-emerald-700 text-white p-6 rounded-xl shadow-sm transition-colors text-center font-semibold flex flex-col items-center justify-center">
            <span className="text-2xl mb-2">📦</span>
            View Inventory
          </Link>
           <Link to="/inventory/restock" className="bg-blue-600 hover:bg-blue-700 text-white p-6 rounded-xl shadow-sm transition-colors text-center font-semibold flex flex-col items-center justify-center">
            <span className="text-2xl mb-2">🔄</span>
            Restock Items
          </Link>
          <Link to="/inventory/add-new" className="bg-amber-500 hover:bg-amber-600 text-white p-6 rounded-xl shadow-sm transition-colors text-center font-semibold flex flex-col items-center justify-center">
            <span className="text-2xl mb-2">➕</span>
            Add Product
          </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h2 className="text-xl font-bold text-gray-800">Recent Sales Overview</h2>
                <p className="text-sm text-gray-500 mt-1">
                  Total Sales: <span className="font-bold text-indigo-600">Rs. {totalPeriodSales.toFixed(2)}</span>
                </p>
              </div>
              <select 
                value={timeFilter} 
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto pr-2">
              {lowStockItems.length === 0 ? (
                <p className="text-gray-500 text-sm text-center mt-10">Inventory looks good! No low stock items.</p>
              ) : (
                  ...
                  {lowStockItems.map(item => (
                    <li key={item.id} className="flex justify-between items-center p-3 border border-red-100 bg-red-50 rounded-lg">
                      ...
                      <div className="text-right">
                        <p className="font-bold text-red-600 text-sm">{item.stock} {item.unit}</p>
                      </div>
                    </li>
                  ))}
                </ul>
