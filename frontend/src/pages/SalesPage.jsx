...
const SalesPage = () => {
  ...
  const handleSearchChange = async (e) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/inventory/search/?q=${value}`);
  ...
  const handleSearchSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/inventory/search/?q=${searchQuery}`);
      const data = await res.json();
      ...
      setMessage({ type: 'error', text: 'Search failed. Make sure backend is running.' });
    }
  };
  ...
  const removeItem = (productId) => {
    setBillItems((prevItems) => prevItems.filter((item) => item.product.id !== productId));
  };
  ...
  const handleCreateBill = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage({ type: 'success', text: `Success! Transaction ID: ${data.transaction_id}` });
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <form onSubmit={handleSearchSubmit} className="flex gap-4 mb-6">
            <input
              ...
              autoFocus
            />
            <button type="submit" className="bg-indigo-600 text-white px-6 py-3 rounded-md font-medium hover:bg-indigo-700">
              Search
            </button>
          </form>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col h-full">
          ...
          {message.text && (
            <div className={`mb-4 p-3 rounded-md text-sm font-medium ${message.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
              {message.text}
            </div>
          )}
          ...
          <div className="flex-1 overflow-y-auto mb-4">
            {billItems.length === 0 ? (
              <p className="text-gray-400 text-center mt-10">No items added yet.</p>
            ) : (
              <ul className="space-y-4">
                {billItems.map((item) => (
                  <li key={item.product.id} className="flex justify-between items-center border-b border-gray-100 pb-3">
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-900">{item.product.name}</p>
                      <p className="text-xs text-gray-500">${item.product.selling_price} x {item.quantity}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => removeItem(item.product.id)} className="text-red-500 hover:text-red-700 text-sm">✖</button>
                    </div>
                  </li>
                ))}
              </ul>
